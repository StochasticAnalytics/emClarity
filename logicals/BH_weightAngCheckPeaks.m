...
% fprintf('Using an angle tolerance/compression factor of %f, %f\n',angleTolerance,compressByFactor);
                     
% Check that the class id is not set to ignore
includedPeaks = reshape(find(positionList(26:26:26*nPeaks) ~= -9999 & positionList(1:26:26*nPeaks) >  0.0),[],1);
excludedPeaks = reshape(find(positionList(26:26:26*nPeaks) == -9999 | positionList(1:26:26*nPeaks) <= 0.0),[],1);
...
% TODO change this to use the angular distance formulat
% acosd((trace(R1*R2')-1)./2)
...
% % Sort with peak # in col one descending on the CCCcycle001_testML_maskedRef-mont_ODD.mrc
...
% I realize these are both caught by the second condition, but I am not
% sure I wnt to actually remove negative scores here. FIXME

peakScore = sortedList(1:26:26*nPeaks);

peakKeep = ( peakWgt~=-9999 & peakScore' > 0 );

peakScore(~peakKeep) = -9999;
% peakWgt(peakKeep) = (peakScore(peakKeep) ./ max(peakScore(peakKeep))).^compressByFactor;

peakWgt(peakKeep) = (peakScore(peakKeep) ./ max(peakScore(peakKeep)));

peakWgt(peakKeep) = (peakWgt(peakKeep) ./ sum(peakWgt(peakKeep))).^compressByFactor;
...
fprintf('raw score [ ');
for iPeak = 1:nPeaks
  fprintf('%1.4f ',peakScore(iPeak));
end
fprintf('] --> [ ');
for iPeak = 1:nPeaks
  fprintf('%1.4f ',peakWgt(iPeak));
end
fprintf(']\n');
...
% % % switch nPeaks 
% % %   case 2
% % %     fprintf('raw score [%1.4f %1.4f] --> [%1.4f %1.4f]\n',peakScore, peakWgt);
% % %   case 3
% % %     fprintf('raw score [%1.4f %1.4f %1.4f] --> [%1.4f %1.4f %1.4f]\n',peakScore, peakWgt);
% % %   case 4
% % %     fprintf('raw score [%1.4f %1.4f %1.4f %1.4f] --> [%1.4f %1.4f %1.4f %1.4f]\n',peakScore, peakWgt);
% % %   case 5
% % %     fprintf('raw score [%1.4f %1.4f %1.4f %1.4f %1.4f] --> [%1.4f %1.4f %1.4f %1.4f %1.4f]\n',peakScore, peakWgt);
% % %   case 6
% % %     fprintf('raw score [%1.4f %1.4f %1.4f %1.4f %1.4f %1.4f] --> [%1.4f %1.4f %1.4f %1.4f %1.4f %1.4f]\n',peakScore, peakWgt);
% % %   otherwise
% % %     error('More than six peaks is a bit too spunky.')
% % % end
