...
try
  shiftInitCutoff = pBH.('shiftInitCutoff');
catch
  shiftInitCutoff = 0;
end
...
% % % for iGPU = 1:nGPUs
  ...
  for iTomo = iterList{iGPU}
    ...
    try
      ...
      % FIXME - this may not be the best choice if template matching
      % results become more statistically robust.
      tmpSearchGeom(:,1) = 0.5; % Level the scores.
      if (doRandom)
        for iRand = 1:size(tmpSearchGeom,1)
          % Replace with random angles.
          tmpSearchGeom(iRand,17:25) = reshape(BH_defineMatrix('rand','Bah','inv'),1,9);
        end        
      end
    catch
...
% % %           tmpSearchGeom(iSubTomo,26:26:26*nPeaks) = -9999;
      ...
      if (shiftInitCutoff)
        cutoffShift = std(overlapMatrix(overlapMatrix > 1))
      else
        cutoffShift = 0;
      end
      

      idxList = positionIDX((overlapMatrix > 1 + cutoffShift));
...
% % %                         subTomoMeta.('ctfGroupSize').(mapName)(2)),1,nPeaks);
    tmpGeom(:,9) = repmat(ceil(tmpGeom(:,11)./ ...
                        subTomoMeta.('ctfGroupSize').(mapName)(2)),1,1);
...
% Randomly divide the data into half sets. Also add random peaks for ML
