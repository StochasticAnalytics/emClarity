...
try
  tilt_geometry_name  = load(tilt_geometry_name);
catch
...
if mod(size(tilt_geometry_name,2),26) ~= 0
...
for iTomo = 1:nTomos
  read_in_Coords = (1 + (iTomo-1)*6: 6 + (iTomo-1)*6);
  ...
  if read_in_Coords(3) > yMax + 75
    error(sprintf('Out of bounds condition for %s yMin at %f, 'please change recon.txt recon.coords', tomoName, read_in_Coords(3), yMax))
  ...
  tomoCoords = struct();
  tomoCoords.('y_i') = int32(read_in_Coords(2));
  tomoCoords.('y_f') = int32(read_in_Coords(3));
  tomoCoords.('NX') = int32(read_in_Coords(tomoIdx, 1));
  tomoCoords.('NY') = int32(read_in_Coords(tomoIdx, 3) - read_in_Coords(tomoIdx, 2) + 1);
  tomoCoords.('NZ') = int32(read_in_Coords(tomoIdx, 4));
  tomoCoords.('dX_specimen_to_tomo') = -1*read_in_Coords(tomoIdx, 5);
  tomoCoords.('dY_specimen_to_tomo') = 
  (emc_get_origin_index(tomoCoords.('NY')) ...
    + read_in_Coords(tomoIdx, 2)) ... % origin of the tomogram in the full tilt projection
    - emc_get_origin_index(tilt_geometry(1,21)), ... % less the origin of the tilt projection = origin of the tomogram wrt the tilt projection
  tomoCoords.('dZ_specimen_to_tomo') = read_in_Coords(tomoIdx, 6);
